...
for i in range(100):
    x=VBA("DPD",["000002.SZ","LST_PRC_RT"])
    Cell(i+1,1).value=x

    y=VBA("EM_REALTIMEQUOT",["000002.SZ","Now"])
    Cell(i+1,2).value=y
...
mktDct = {'SH':['A', frml_AHis, r'D:\data\choice\his\sh.csv'],
          'SZ':['A', frml_AHis, r'D:\data\choice\his\sz.csv'],
          'HK':['HK', frml_HKHis, r'D:\data\choice\his\hk.csv'],
    ...
    for mkt  in ['SH', 'SZ']: #, 'HK', 'A', 'N', 'O']:
        ...
        for x in prdOfMkt:
            ...
            if dfDct[mkt].columns <> []:
                if x in dfDct[mkt]['pid'].values:
                    dbDate = str( dfDct[mkt].query('pid=="%s"'%x)['date'].max() )
            ...
            clear_sheet( shtName )
            Cell(shtName,1,1).formula = frml % x
            recalc_sheet(shtName)
            Cell(shtName,1,7).value = x
            ...
            dfPrd.columns = ACols
            dfPrd.to_csv(r'D:\data\choice\his\%s.csv' % x)
