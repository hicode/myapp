...
insert into myapp_kdaily_cns1 select * from myapp_kdaily_cns order by product_id, date
ALTER TABLE myapp_kdaily_cns RENAME TO myapp_kdaily_cns2; ALTER TABLE myapp_kdaily_cns1 RENAME TO myapp_kdaily_cns;
insert into myapp_kdaily_cns select id, p, o, h, l, c, chngPerc, adjC, amt, vol, product_id, date, pDate,  strftime('%w', date) weekday, strftime('%W', date) week, strftime('%m', date) month, strftime('%Y', date) year from myapp_kdaily_cns1;
update myapp_kdaily_cns set weekday = strftime('%w', date), week = strftime('%W', date), month = strftime('%m', date), year = strftime('%Y', date);
update myapp_kdaily_cns1 set h=h*adjC/c, l=l*adjC/c, o=o*adjC/c, amt=c, c=adjC, adjC=amt; # , c=adjC;  # some adjC isnull lead to fail of update
...
#ALTER TABLE myapp_kdaily ADD COLUMN column_def...
...
delete from periodHL;
insert into periodHL(product_id, h, l) select product_id, max(h), min(l) from myapp_kdaily_cns1 where year=2015 and month>5 group by product_id;
update myapp_productposition set h3Mon=(select h from periodHL where myapp_productposition.product_id=periodHL.product_id);
update myapp_productposition set l3Mon=(select l from periodHL where myapp_productposition.product_id=periodHL.product_id);

delete from periodHL;
insert into periodHL(product_id, h, l) select product_id, max(h), min(l) from myapp_kdaily_cns1 where year=2014 group by product_id;
update myapp_productposition set h2Year=(select h from periodHL where myapp_productposition.product_id=periodHL.product_id);
update myapp_productposition set l2Year=(select l from periodHL where myapp_productposition.product_id=periodHL.product_id);

delete from periodHL;
insert into periodHL(product_id, h, l) select product_id, max(h), min(l) from myapp_kdaily_cns1 where year=2013 group by product_id;
update myapp_productposition set h3Year=(select h from periodHL where myapp_productposition.product_id=periodHL.product_id);
update myapp_productposition set l3Year=(select l from periodHL where myapp_productposition.product_id=periodHL.product_id);
