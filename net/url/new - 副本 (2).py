...
def tickDataFromCsv(fn, pid, conn):
    cur = conn.cursor()
    #import contextlib
    try:
...
def getHist(prodDictNoHist8Submarket, conn):  # http://ichart.yahoo.com/table.csv?s=600000.SS&a=01&b=01&c=1990&d=08&e=30&f=2015&g=d 
    cur = conn.cursor()
    prodDictWithHist = {}
    for key in prodDictNoHist8Submarket.keys():
        ...
        for prod in prodDictNoHist8Submarket[key]:
            ...
            for ln in lines[1:]:
                ...
                if len(fLst)<7:
                    continue
                if fLst[0] <= p['dateHistEnd']:
                    if fLst[0] >= p['dateHistBegin']:
                        continue;
...
def getQianLongDailyRpt(fn, conn): #, market):
    ...
    for ln in rslt[1:]:
        ...
        #    continue
        if p['dateHistBegin<>None']:
            if dateStr > p['dateHistEnd'].strftime('%Y-%m-%d') or dateStr < p['dateHistBegin'].strftime('%Y-%m-%d'):
                histRec.append( [ p['id'] ] + [dateStr, fld[11], fld[12], fld[13], fld[4], fld[6]] )  
        else:
            pass
    try:
        cur.executemany( "insert into %s(product_id, date, o, h, l, c, vol) values (?,?,?,?,?,?,?)" % tblName, histRec )
        #cur.executemany( "insert into myapp_KDaily(code, market, date, o, h, l, c, vol, adjC) values (?,?,?,?,?,?,?,?,?)", histRec )
    except sqlite3.Error,e:
...
def prepareTrading():
    ...
    for prod in prodAll:
        prodIdMap[prod.id] = {'code':prod.code, 'market':prod.market, 'bHist':prod.bDataHist, 'dateHistBegin':prod.dateHistBegin, 'dateHistEnd':prod.dateHistEnd, 'type':prod.type}
        prodMapId[prod.code+'.'+prod.market] = {'id':prod.id, 'bHist':prod.bDataHist, 'dateHistBegin':prod.dateHistBegin, 'dateHistEnd':prod.dateHistEnd }
...
t = time.clock()
getDzhCodeLst('D:\data\codeBook\SZ150926.SNT', 'SZ')
getDzhCodeLst('D:\data\codeBook\SH150926.SNT', 'SH')
#getDzhCodeLst('D:\data\codeBook\HK150926.SNT', 'HK')
...
fl = [r'D:\data\dailydzh\20150922.txt', r'D:\data\dailydzh\20150923.txt', r'D:\data\dailydzh\20150924.txt', r'D:\data\dailydzh\20150925.txt']
fl.sort()
#for f in fl:
#    getQianLongDailyRpt(f, conn)
#postImportData(conn)
...
t = time.clock()
#getHist(prodDictNoHist8Submarket, conn)
